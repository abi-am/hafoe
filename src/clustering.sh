...
# Cluster filtered ORF sequences using cd-hit

while getopts h:i:o:p:l:c:n:g:a: flag
do
    case "${flag}" in
        h) cd_hit_est_path=${OPTARG};;
        i) input_path=${OPTARG};;
        o) output_dir=${OPTARG};;
...
echo -ne "Command: $cd_hit_est_path \n\t -i $input_path \n\t -o $output_dir/$output_prefix \n\t -g 1 \n\t -d 100 \n\t -c $seq_identity \n\t -n $word_length \n\t -G $identity_type \n\t -aL $aln_coverage \n" 
echo -ne "cd-hit-est path: $cd_hit_est_path \n"
